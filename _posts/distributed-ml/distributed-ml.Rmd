...
We'll conclude with how the parallelisms are composed together in practice by looking at Llama 3.
...
It's useful to first get an understanding of how data flows during the PyTorch model training process, in order to see the opportunities for parallelisation. 
...
As of PyTorch 2.5, most of the parallelisms offered only exists in eager mode -- which, as we'll see, can often lead to suboptimal sequences of operations.
...
The example we used in [Section 1](#how-do-you-train-a-pytorch-neural-network), a **multi-layer perceptron** (MLP), consists of a GEMM followed by a pointwise pointwise nonlinear activation function $\sigma(\cdot)$:
$$ 
Y = \sigma(XA)
$$
