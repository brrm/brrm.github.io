...
Though Ring AllReduce is bandwidth optimal, its end-to-end latency scales linearly with the number of ranks. A lower latency, tree-based alternative is discussed in [another post]().
...
#### DDP in PyTorch
<!-- TODO: how do we actually write code that uses DDP? -->
PyTorch's distribution API is designed for non-intrusive scaling out from local training. 
Applying DDP to a model is as simple as wrapping our local model with the DDP `nn.Module` class: `nn.parallel.DDP(model, device_ids=[rank])`.
...
Notice in the above that gradients for inner layers are already available while we're still computing the backward pass of the more outer layers.
...
PyTorch implements this by registering one autograd hook with each parameter tensor, which fires after the corresponding gradients are updated.
...
DDP's younger brother, FSDP, addresses this by also splitting model parameters.
FSDP is similar to DeepSpeed's ZeRO [@zero2020], but designed to better align with the other components of PyTorch.

FSDP reduces memory footprint by <mark>sharding</mark> model parameters: the model is split *horizontally* so that each rank only holds a subset (**"shard"**) of the parameters (and associated gradients and optimizer state) in a given layer.
...
However, we need to be able to materialise parameters on a single GPU, eliminating our memory savings!
...
<!-- TODO: maybe our own example, with our own (animated?) diagram -->
A simple six layer model, for example, could be decomposed into three units: `[layer0, layer3]`, `[layer1, layer2]` and `[layer4, layer5]`.
Consider what happens to `unit1` consisting of `[layer1, layer2]`:

1. Before the forward pass through `layer1`, we materialise the parameters in `unit1` by gathering shards from other peer ranks. We can do this with an AllGather (equivalent to each rank Broadcasting its shard).
...
4. After gradients are calculated, we free peer shards and then ReduceScatter to average and shard gradients (equivalent to each rank Reducing the gradients in its shard).
...
By setting $F=1$, FSDP fully replicates the model (with no memory savings) and simplifies to vanilla DDP using AllReduce for gradient synchronisation.
By setting $F=W$ (i.e. the global world size), FSDP fully shards the model with each rank only holding $\frac{1}{W}$ of the model (the previous six layer example is fully sharded with $F=W=2$).
...
It's worth noting that with any sharding strategy, ranks will always have distinct input batch chunks (otherwise we'd simply be duplicating gradient calculations). 
...
Clearly communication within the sharding group is much more expensive, therefore we often try to minimise the number of hops between the ranks in a sharding group -- sometimes we may even use smaller sharding factors to guarantee they're within the same host.

#### FSDP in PyTorch
NCCL AllGather even-ness + collective sizes (figure 2), FlatParameter, number of units trades-off memory and comm, Autograd, when we gather shards what's included (params, grads, optimizer states)?
